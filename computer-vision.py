...
gf_output_path = "data/" + sys.argv[1].split('.')[0] + '-gray.' + sys.argv[1].split(".")[1]
bf_output_path = "data/" + sys.argv[1].split('.')[0] + '-blur.' + sys.argv[1].split(".")[1]
cf_output_path = "data/" + sys.argv[1].split('.')[0] + '-canny.' + sys.argv[1].split(".")[1]
mi_output_path = "data/" + sys.argv[1].split('.')[0] + '-mask.' + sys.argv[1].split(".")[1]
li_output_path = "data/" + sys.argv[1].split('.')[0] + '-line.' + sys.argv[1].split(".")[1]
ci_output_path = "data/" + sys.argv[1].split('.')[0] + '-final.' + sys.argv[1].split(".")[1]
...
mi_out = cv2.VideoWriter(mi_output_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, size, 0)
...
while(video.isOpened()):
    ...
    hough = cv2.HoughLinesP(masked_image, 2, np.pi / 180, 100, np.array([]), minLineLength = 100, maxLineGap = 50)
